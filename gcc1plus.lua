...
local function get_cc1plus_command(test_file, extra_args)
  ...
  if not gcc_root then return nil end
...
vim.api.nvim_create_user_command('GdbCC1plus', function(opts)
  ...
  local gcc_root = validate_gcc_env()
  if not gcc_root then return end
...
vim.api.nvim_create_user_command('ShowTestLog', function()
  ...
  if not log_file then
    vim.notify(
      'Test log not found. Run :RunTestsuite first to generate logs.',
      vim.log.levels.WARN
    )
    return
  end
...
vim.api.nvim_create_user_command('RunTest', function(opts)
  ...
  vim.notify('Compiling: ' .. test_name .. (dejagnu_opts ~= '' and ' (with options: ' .. dejagnu_opts .. ')' or ''), vim.log.levels.INFO)
  vim.cmd('terminal ' .. cmd)
end, { nargs = 1, complete = 'file' })
